...
namespace AvalonDockTest.FlaUITests
{
    ...
    public abstract class FlaUITestBase
    {
        ...
        protected void CloseDocument(string documentName, bool confirmClose = true)
        {
            ...
            if (confirmClose)
                DismissAnyDialogIfPresent("Yes", "Ja");
        }
        ...
        {
            var result = Retry.WhileNull(
                () => { var w = GetFloatingWindows(); return w.Length > 0 ? w[0] : null; },
                timeout: TimeSpan.FromSeconds(timeoutSeconds),
        ...
        protected string DismissAnyDialogIfPresent(params string[] preferredButtonNames)
        {
            ...
            var title = ""; try { title = dialog.Name; } catch { }
        ...
        {
            try { dialog.Focus(); System.Threading.Thread.Sleep(100); } catch { }
            ...
            {
                dismissBtn = buttons.FirstOrDefault(b => { try { return b.Name == name; } catch { return false; } });
                if (dismissBtn != null) break;
            }

            if (dismissBtn != null)
            {
                try { dismissBtn.Click(true); Wait.UntilInputIsProcessed(); System.Threading.Thread.Sleep(300); } catch { }
                if (!IsDialogStillOpen(dialog)) return;
                try { dismissBtn.Patterns.Invoke.PatternOrDefault?.Invoke(); Wait.UntilInputIsProcessed(); System.Threading.Thread.Sleep(300); } catch { }
                if (!IsDialogStillOpen(dialog)) return;
            }
        ...
        {
            try { return dialog.Properties.ProcessId.IsSupported && !dialog.IsOffscreen; }
            catch { return false; }
        }
        ...
        {
            var result = Retry.WhileNull(
                () => { try { return App.GetMainWindow(Automation, TimeSpan.FromSeconds(2)); } catch { return null; } },
