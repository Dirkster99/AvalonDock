...
Set `serializer.UnresolvedContentHandling = UnresolvedContentHandling.Hide` if you would rather keep unresolved anchorables in `LayoutRoot.Hidden`, with the pane and index they were stored at, so that content supplied later can still restore them in place. The same property on `args` overrides this for a single item, which is what a plugin host wants: park the tool windows whose plugin has not loaded yet, drop the rest. `args.Cancel = true` removes the item either way.
